...
ParameterizedTestParameters(stats_query, test_stats_query_get_aggregated)
{
  static QueryTestCase test_cases[] =
  {
    {"*.*", "0\n"},
...
ParameterizedTest(QueryTestCase *test_cases, stats_query, test_stats_query_get_aggregated)
{
  GString *result = NULL;

  _initialize_counter_hash();

  result = stats_query_get_aggregated(test_cases->pattern);
  cr_assert_str_eq(result->str, test_cases->expected,
